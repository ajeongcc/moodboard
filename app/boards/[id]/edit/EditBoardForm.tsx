...
function VideoCard({
  ...
  const platformLabel = { youtube: 'YouTube', vimeo: 'Vimeo', soundcloud: 'SoundCloud', spotify: 'Spotify', applemusic: 'Apple Music' }[video.platform]
  const isAudio = video.platform === 'soundcloud' || video.platform === 'spotify' || video.platform === 'applemusic'

  return (
    <div className={`relative group rounded-xl overflow-hidden bg-zinc-100 dark:bg-zinc-800 ${isAudio ? 'aspect-[3/1]' : 'aspect-video'}`}>
      {thumbnail ? (
        ...
        <div className="w-full h-full flex flex-col items-center justify-center gap-2">
          ...
          {video.platform === 'spotify' && (
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-green-400">
              <circle cx="12" cy="12" r="10"/>
              <path d="M8 11.5c2.5-1 5.5-1 8 0M7 14.5c3-1.2 7-1.2 10 0M9.5 17.5c2-0.7 5-0.7 7 0"/>
            </svg>
          )}
          {video.platform === 'applemusic' && (
            /* Apple Music 핑크 음표 아이콘 */
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-pink-400">
              <path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/>
            </svg>
          )}
          {!isAudio && (
...
export default function EditBoardForm({ boardId, initialTitle, initialDescription, initialIsPublic, initialImages, initialVideos }: Props) {
  ...
  // 음악 URL 추가 (SoundCloud / Spotify / Apple Music)
  const AUDIO_PLATFORMS = ['soundcloud', 'spotify', 'applemusic'] as const
  function handleAddAudio() {
    setAudioInputError('')
    const parsed = parseVideoUrl(audioInput)
    if (!parsed || !(AUDIO_PLATFORMS as readonly string[]).includes(parsed.platform)) {
      setAudioInputError('사운드클라우드, 스포티파이, 또는 애플 뮤직 링크를 입력해주세요.')
      return
    }
    setNewVideos((prev) => [...prev, {
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="사운드클라우드, 스포티파이, 애플 뮤직 링크를 붙여넣으세요"
                className="flex-1 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-4 py-2.5 text-sm text-zinc-900 dark:text-zinc-50 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 outline-none focus:border-zinc-900 dark:focus:border-zinc-400 focus:ring-2 focus:ring-zinc-900/10 dark:focus:ring-zinc-400/20 transition"
              />
            ...
            {/* soundcloud 카드만 표시 */}
            {[...existingVideos, ...newVideos].filter(v => v.platform === 'soundcloud' || v.platform === 'spotify' || v.platform === 'applemusic').length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {existingVideos.filter(v => v.platform === 'soundcloud' || v.platform === 'spotify' || v.platform === 'applemusic').map((v) => (
                  <VideoCard key={v.id} video={v} onRemove={handleRemoveExistingVideo} />
                ))}
                {newVideos.filter(v => v.platform === 'soundcloud' || v.platform === 'spotify' || v.platform === 'applemusic').map((v) => (
                  <VideoCard key={v.tempId} video={v} onRemove={handleRemoveNewVideo} />
                ))}
              </div>
            )}
