...
function VideoCard({
  ...
  const platformLabel = { youtube: 'YouTube', vimeo: 'Vimeo', soundcloud: 'SoundCloud', spotify: 'Spotify' }[video.platform]
  const isAudio = video.platform === 'soundcloud' || video.platform === 'spotify'
...
export default function EditBoardForm({ boardId, initialTitle, initialDescription, initialImages, initialVideos }: Props) {
  ...
  // 음악 URL 추가 (SoundCloud / Spotify)
  function handleAddAudio() {
    setAudioInputError('')
    const parsed = parseVideoUrl(audioInput)
    if (!parsed || (parsed.platform !== 'soundcloud' && parsed.platform !== 'spotify')) {
      setAudioInputError('사운드클라우드 또는 스포티파이 링크를 입력해주세요.')
      return
    }
    setNewVideos((prev) => [...prev, {
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="사운드클라우드 또는 스포티파이 링크를 붙여넣으세요"
                className="flex-1 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-4 py-2.5 text-sm text-zinc-900 dark:text-zinc-50 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 outline-none focus:border-zinc-900 dark:focus:border-zinc-400 focus:ring-2 focus:ring-zinc-900/10 dark:focus:ring-zinc-400/20 transition"
              />
            ...
            {/* soundcloud 카드만 표시 */}
            {[...existingVideos, ...newVideos].filter(v => v.platform === 'soundcloud' || v.platform === 'spotify').length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {existingVideos.filter(v => v.platform === 'soundcloud' || v.platform === 'spotify').map((v) => (
                  <VideoCard key={v.id} video={v} onRemove={handleRemoveExistingVideo} />
                ))}
                {newVideos.filter(v => v.platform === 'soundcloud' || v.platform === 'spotify').map((v) => (
                  <VideoCard key={v.tempId} video={v} onRemove={handleRemoveNewVideo} />
                ))}
              </div>
            )}
